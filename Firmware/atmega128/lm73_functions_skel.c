...
//Yeah, this is for you to do! ;^)
	...
	temp_digits[4] = int_to_char((temp / 10) % 10); // tenths
	temp_digits[5] = int_to_char(temp % 10); // hundredths
	...
	if(temp_digits[0] == '0') //if hundreds is 0
	{
		...
		if(temp_digits[1] == '0') //if tens char is also zero
		{
			...
			{
				temp_digits[1] = ' '; //set tens char to blank space
			}
		}
	}

	return negative_flag; //return 1 if need to insert negative symbol before occurence of temp_digits
				//this can only return 1 if the temperature is a negative number with 3 digits (ex: -100)
